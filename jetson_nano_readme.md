...
docker run -it --runtime nvidia --network host --gpus all -e DISPLAY dustynv/ros:foxy-ros-base-l4t-r32.7.1
```
...
### Prerequisites
   ...
   * Ubuntu 20.04 (Jetson Nano compatible)
   * ROS2 Foxy
...
For headless operation on Jetson Nano:

1. Disable GUI services to save resources:
```bash
sudo systemctl set-default multi-user.target
```

2. Configure environment variables for headless operation:
```bash
export DISPLAY=:0
export QT_X11_NO_MITSHM=1
```

3. For remote access, use SSH with X11 forwarding:
```bash
ssh -X username@jetson_ip
```
