...
class CVProcessor:
    def __init__(self, propeller_mask_height_ratio, propeller_mask_width_ratio, vertical_offset_ratio):
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/aniketh/programming/Lancelot/paraceptor_ws/src/paraceptor/yolov5/weights/best_fixed.pt')
    ...
    def process_image(self, current_frame, current_yaw):
        ...
        recon_centroid_x, recon_centroid_y, highest_conf, distance_to_target = self.get_centroid_and_distance(results, height, width)
    ...
    def get_centroid_and_distance(self, results, height, width):
        ...
        return best_centroid_x, best_centroid_y, highest_conf, distance_to_target

    def calculate_target_position(self, centroid_x, centroid_y, distance, image_width, image_height, current_yaw):
        # Convert pixel coordinates to normalized device coordinates
        x_ndc = (centroid_x - image_width / 2) / (image_width / 2)
        y_ndc = (centroid_y - image_height / 2) / (image_height / 2)

        # Convert normalized device coordinates to angles (in radians)
        x_angle = np.arctan(x_ndc)
        y_angle = np.arctan(y_ndc)

        # Calculate target position in the drone's local frame (before yaw rotation)
        target_x = distance * np.cos(y_angle) * np.cos(x_angle)  # Forward/Backward (North)
        target_y = distance * np.cos(y_angle) * np.sin(x_angle)  # Left/Right (East)
        target_z = -distance * np.sin(y_angle)  # Up/Down (Down)

        # Rotate target position based on current yaw (NED)
        yaw_rad = np.radians(current_yaw)
        target_x_rot = target_x * np.cos(yaw_rad) + target_y * np.sin(yaw_rad)
        target_y_rot = -target_x * np.sin(yaw_rad) + target_y * np.cos(yaw_rad)

        return target_x_rot, target_y_rot, target_z
