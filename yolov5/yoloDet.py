...
class YoloTRT():
    ...
    def Inference(self, img):
        ...
        # Use existing context and memory
        stream = cuda.Stream()
        cuda.memcpy_htod_async(self.cuda_inputs[0], self.host_inputs[0], stream)

        t1 = time.time()
        # Execute inference on the GPU
        self.context.execute_async(bindings=self.bindings, stream_handle=stream.handle)
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.PostProcess(
                output[i * self.LEN_ALL_RESULT: (i + 1) * self.LEN_ALL_RESULT], origin_h, origin_w
            )
